...
class GdaxMarket implements Market {
  currency: Currency
  orders: Orders
  accounts: Accounts
  ...
  async init() {
    ...
    const websocketURI = this.sandbox
      ? config.api.sandboxWebsocketURI
      : config.api.websocketURI
    const websocketAuth = this.sandbox
      ? null
      : {
  ...
  watchCurrencyPrice() {
    if (!this.initialized) {
      throw new Error(
        "GdaxMarket #watchCurrencyPrice() cannot be used without being initialized"
      )
    }
  ...
  async getCurrencyPrice() {
    if (!this.initialized) {
      throw new Error(
        "GdaxMarket #getCurrencyPrice() cannot be used without being initialized"
      )
    }
