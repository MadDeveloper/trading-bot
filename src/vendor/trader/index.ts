...
class Trader implements Trading {
  ...
  async updateBalances() {
    this.quoteCurrencyBalance = await this.accounts.availableFunds(
      this.quoteCurrency
    )
    this.baseCurrencyBalance = await this.accounts.availableFunds(
      this.baseCurrency
    )
    this.verifyBalances()
  ...
  private verifyBalances() {
    if (
      this.quoteCurrencyBalance < config.trader.minQuantityQuoteCurrencyToUse
    ) {
      Logger.debug(
        `WARN: The ${this.quoteCurrency} balance (${this.quoteCurrencyBalance} ${this.quoteCurrency}) is under the min quantity desired to use (${config.trader.minQuantityQuoteCurrencyToUse} ${this.quoteCurrency})`
      )
    } else if (
      this.quoteCurrencyBalance > config.trader.maxQuantityQuoteCurrencyToUse
    ) {
      Logger.debug(
        `The ${this.quoteCurrency} balance (${this.quoteCurrencyBalance} ${this.quoteCurrency}) is over the max quantity desired to use (${config.trader.maxQuantityQuoteCurrencyToUse} ${this.quoteCurrency})`
      )
  ...
  async trade() {
    ...
    try {
      ...
      Logger.error(
        `Fatal error occured while trying to retrieve account balances: ${JSON.stringify(
          error,
          null,
          2
        )}`
  ...
  watchChartWorker() {
    this.workObserver = this.chartWorker.work$.subscribe(
      async (work: ChartWork) => {
  ...
  async analyzeWorks() {
    ...
    if (TraderState.WAITING_TO_BUY === this.state) {
      ...
      Logger.debug(
        "Trader tried to analyze next work before waiting api response for the last request. Please check your network latency."
      )
    } else {
  ...
  async buyStrategies() {
     ...
     */
    if (
      this.chartAnalyzer.detectHollow(
        this.works
      ) /* || this.chartAnalyzer.isUpwardTrendConfirmed(this.lastWork, this.works)*/
    ) {
      const funds = this.fundsToUse()

      Logger.debug(
        `Trader is buying at ${this.lastWork.price} ${this.quoteCurrency}`
      )
  ...
  async sellStrategies() {
    ...
    if (
      config.trader.sellWhenPriceExceedsMaxThresholdOfProfitability &&
      this.lastWork.price >=
        Equation.maxThresholdPriceOfProfitability(this.lastBuyTrade.price)
    ) {
      /*
       ...
       */
      Logger.debug(
        `Max threshold of profitability reached (profitability: ${config.trader.maxThresholdOfProfitability}%)`
      )
      Logger.debug(`Trader is selling at ${this.lastWork.price}`)
      await this.sell(size)
    } else if (
      config.trader.sellWhenPriceExceedsMinThresholdOfProfitability &&
      Equation.isProfitable(this.lastBuyTrade.price, this.lastWork.price) &&
      !Equation.isProfitable(this.lastBuyTrade.price, this.lastWork.lastPrice)
    ) {
      /*
       ...
       */
      Logger.debug(
        `Min threshold of profitability reached (profitability: ${config.trader.minThresholdOfProfitability}%)`
      )
      Logger.debug(`Trader is selling at ${this.lastWork.price}`)
      ...
      let partSizeToSell =
        size *
        (config.trader
          .quantityToSellWhenPriceExceedsMinThresholdOfProfitability /
          100)
      const minQuantity = this.market.orders.getMinQuantity()

      Logger.debug(
        `Quantity desired to sell: ${partSizeToSell} ${this.baseCurrency} (${config.trader.quantityToSellWhenPriceExceedsMinThresholdOfProfitability}%)`
      )
      ...
      try {
        ...
        const quantityRemaining = this.market.orders.normalizeQuantity(
          size - partSizeToSell
        )
        ...
        if (quantityRemaining < minQuantity) {
          ...
          Logger.debug(
            `If we sell ${config.trader.quantityToSellWhenPriceExceedsMinThresholdOfProfitability}% of the quantity, the remaining quantity is below the minQuantity. Selling all the quantity`
          )
          partSizeToSell = size
          this.clearWorksAfterTrade = true
        }
        ...
        Logger.debug(
          `${config.trader.quantityToSellWhenPriceExceedsMinThresholdOfProfitability}% of the quantity is below the minQuantity. Taking the minQuantity as quantity to sell`
        )
      ...
      config.trader.useExitStrategyInCaseOfLosses &&
      Equation.rateBetweenValues(
        this.lastBuyTrade.price,
        this.lastWork.price
      ) <= -config.trader.sellWhenLossRateReaches
    ) {
      /*
      ...
      !this.chartWorker.isInFastMode() &&
      this.chartAnalyzer.detectProfitablePump(
        this.works,
        this.lastBuyTrade.price
      )
    ) {
  ...
  fundsToUse(): number {
    const funds =
      (config.trader.quantityOfQuoteCurrencyToUse / 100) *
      this.quoteCurrencyBalance
    ...
    if (funds < config.trader.minQuantityQuoteCurrencyToUse) {
      if (
        config.trader.minQuantityQuoteCurrencyToUse > this.quoteCurrencyBalance
      ) {
        Logger.debug(
          `Trader has not enough funds (minQuantityQuoteCurrencyToUse is not respected: ${config.trader.minQuantityQuoteCurrencyToUse}, current balance: ${this.quoteCurrencyBalance})`
        )
        this.stop()
      }
  ...
  sizeToUse(): number {
    return (
      (config.trader.quantityOfBaseCurrencyToUse / 100) *
  ...
  async buy(funds: number) {
    try {
      ...
      const order = await this.market.orders.buyMarket(
      ...
      Logger.debug(`
            ...
            `)
      Logger.debug(
        `Last buy trade: ${JSON.stringify(this.lastBuyTrade, null, 2)}`
      )
      Logger.debug(
  ...
  async sell(size: number, partial?: boolean) {
    try {
      ...
      const order = await this.market.orders.sellMarket(
        this.market.currency,
        size,
        lastWorkBackup.price
      )
      ...
      this.lastSellTrade = {
        ...
        time: lastWorkBackup.time,
        benefits:
          Equation.rateBetweenValues(this.lastBuyTrade.price, price) -
  ...
  private async loadData() {
    ...
    try {
      ...
      if (dataString && dataString.length > 0) {
        ...
        const lastTrade =
          this.trades.length > 0 ? this.trades[this.trades.length - 1] : null
        ...
        if (lastTrade) {
          ...
          if (
            TradeType.BUY === lastTrade.type ||
            TradeType.SELL_PARTIAL === lastTrade.type
          ) {
            this.state = TraderState.WAITING_TO_SELL
          } else {
            this.state = TraderState.WAITING_TO_BUY
          ...
          Logger.debug(
            `Starting from a previous trade: ${JSON.stringify(
              lastTrade,
              null,
              2
            )}`
  ...
  private findLastBuyTrade(trades: Trade[]): Trade {
    return trades
      .slice()
      .reverse()
      .find((trade) => trade.type === TradeType.BUY)
  }
  ...
  private findLastSellTrade(trades: Trade[]): Trade {
    return trades
      ...
      .find(
        (trade) =>
          trade.type === TradeType.SELL || trade.type === TradeType.SELL_PARTIAL
      )
  }
  ...
  private async persistData() {
    try {
      ...
      await write("./data.json", JSON.stringify(this.forgeData(), null, 2), {
        encoding: "utf-8",
      })
    } catch (error) {
  ...
  private forgeData(): DataStorage {
    return {
      ...
      worksSmoothed: this.chartWorker.filterNoise(
        this.chartWorker.copyWorks(this.chartWorker.allWorks)
      ),
      trades: this.trades,
      nextMinProfitablePrice:
        this.state === TraderState.WAITING_TO_SELL
          ? Equation.thresholdPriceOfProfitability(this.lastBuyTrade.price)
          : null,
      nextMaxProfitablePrice:
        this.state === TraderState.WAITING_TO_SELL
          ? Equation.maxThresholdPriceOfProfitability(this.lastBuyTrade.price)
          : null,
      nextPanicSellPrice:
        this.state === TraderState.WAITING_TO_SELL
          ? this.lastBuyTrade.price *
            (100 - config.trader.sellWhenLossRateReaches)
          : null,
