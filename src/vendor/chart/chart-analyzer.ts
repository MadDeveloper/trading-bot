...
class ChartAnalyzer {
  ...
  detectHollow(works: ChartWork[]): boolean {
    ...
    works.forEach((work) => {
      if (
        ...
        this.isUpwardTrendConfirmed(work, works) &&
        !this.ignorePump(work)
      ) {
        upwardTrendConfirmed = true
      } else if (
        !downwardTrendConfirmed &&
        this.isDownwardTrendConfirmed(work, works)
      ) {
    ...
    Logger.debug(
      `\nHollow detected at: ${
        (Number(downwardTrendConfirmed) / 2 +
          Number(upwardTrendConfirmed) / 2) *
        100
      }%`
    )
  ...
  detectProfitablePump(works: ChartWork[], buyPrice: number): boolean {
    const thresholdPriceProfitable = Equation.thresholdPriceOfProfitability(
      buyPrice
    )
    ...
    works.forEach((work) => {
      if (!profitablePump) {
        profitablePump =
          work.price >= thresholdPriceProfitable &&
          this.isUpwardTrendConfirmed(work, works)
      }
    })
  ...
  isDownwardTrendConfirmed(work: ChartWork, works: ChartWork[]): boolean {
    return (
      this.isBigDumpConfirmed(work) || this.trendsConfirmDownward(work, works)
    )
  }

  isUpwardTrendConfirmed(work: ChartWork, works: ChartWork[]): boolean {
    return (
      this.isBigPumpConfirmed(work) || this.trendsConfirmUpward(work, works)
    )
  }

  ignorePump(work: ChartWork): boolean {
    return (
      config.chart.ignoreBigPumpWhenBuying &&
      Math.abs(Equation.rateBetweenValues(work.lastPrice, work.price)) >
  ...
  isBigDumpConfirmed(work): boolean {
    return (
      config.chart.validateDumpWhenBigDumpIsDetected &&
      work.trend === Trend.DOWNWARD &&
      this.rateBetweenPricesConfirmTrend(work.lastPrice, work.price)
    )
  }

  isBigPumpConfirmed(work): boolean {
    return (
      config.chart.validatePumpWhenBigPumpIsDetected &&
      work.trend === Trend.UPWARD &&
      this.rateBetweenPricesConfirmTrend(work.lastPrice, work.price)
    )
  }
  ...
  trendsConfirmDownward(work: ChartWork, works: ChartWork[]): boolean {
    ...
    for (
      let index = 0;
      index < config.chart.numberOfDownPointsToValidateDump;
      index++
    ) {
      if (lastWork) {
  ...
  trendsConfirmUpward(work: ChartWork, works: ChartWork[]): boolean {
    ...
    for (
      let index = 0;
      index < config.chart.numberOfUpPointsToValidatePump;
      index++
    ) {
      if (lastWork) {
  ...
  rateBetweenPricesConfirmTrend(priceA, priceB) {
    ...
    return (
      Math.abs(Equation.rateBetweenValues(priceA, priceB)) >=
      config.chart.thresholdRateToApproveInversion
    )
  }
}
