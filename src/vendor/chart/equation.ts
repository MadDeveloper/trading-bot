...
class Equation {
  ...
  static isProfitableOnQuantity(
    quoteCurrencyInvested,
    size,
    sellPrice
  ): boolean {
    return (
      size * sellPrice * (1 - config.market.orderFees) > quoteCurrencyInvested
    )
  }

  static thresholdPriceOfProfitability(
    buyPrice: number,
    threshold: number = config.trader.minThresholdOfProfitability
  ): number {
    const multiplierFeesIncluded = 1 - config.market.orderFees

    if (multiplierFeesIncluded === 0) {
      throw new Error(
        `Fees cannot be 100%. Mathematic error when trying to calculate threshold price of profitability (multiplierFeesIncluded = 0, cannot divide by zero)`
      )
    }
    ...
    return (
      buyPrice * ((1 + threshold / 100) / Math.pow(multiplierFeesIncluded, 2))
    )
  }

  static maxThresholdPriceOfProfitability(buyPrice: number): number {
    return Equation.thresholdPriceOfProfitability(
      buyPrice,
      config.trader.maxThresholdOfProfitability
    )
  }
}
