...
class ChartWorker {
  ...
  workOnPriceTicker() {
    ...
    this.tickerIntervalRef = setInterval(
      () => this.priceTickerWork(),
      this.tickerInterval
    )
  }
  ...
  async priceTickerWork() {
    ...
    try {
      ...
      Logger.debug(
        "Worker has retrieved the currency price, it continues the work"
      )
    }
  ...
  fastMode() {
    if (this.mode !== WorkerSpeed.FAST) {
      ...
      this.changeTickerInterval(
        config.chart.tickerInterval *
          (1 - config.chart.reductionOfTheTickerIntervalOnSpeedMode)
      )
    }
  }
  ...
  computeTrend(lastPoint, newPoint): number {
    const leadingCoefficient = Equation.findLineLeadingCoefficient(
      lastPoint,
      newPoint
    )
  ...
  filterNoise(works: ChartWork[]): ChartWork[] {
    ...
    switch (smooting) {
      ...
      default:
        return this.removePointsTooClose(
          this.removeIsolatedBumpAndHollow(works)
        )
    }
  }
  ...
  smoothMovingAverage(works: ChartWork[]): ChartWork[] {
    ...
    return works.map((work, index) => {
      ...
      if (previousWork && nextWork) {
        smoothedWork.price =
          (previousWork.price + work.price + nextWork.price) / 3
      }

      if (previousWork) {
        smoothedWork.lastTrend = previousWork.trend
        smoothedWork.trend = this.computeTrend(
          this.extractPointFromWork(previousWork),
          this.extractPointFromWork(smoothedWork)
        )
      }
  ...
  removePointsTooClose(works: ChartWork[]): ChartWork[] {
    ...
    return works.filter((work, index) => {
      ...
      if (
        Math.abs(Equation.rateBetweenValues(lastWorkKept.price, work.price)) >
        ...
        const newTrend = this.computeTrend(
          pointFromLastWorkKept,
          pointFromCurrentWork
        )
  ...
  removeIsolatedBumpAndHollow(works: ChartWork[]): ChartWork[] {
    ...
    return works.filter((work, index) => {
      ...
      const isolatedBump =
        previousWork.trend === Trend.DOWNWARD &&
        work.trend === Trend.UPWARD &&
        nextWork.trend === Trend.DOWNWARD
      const isolatedHollow =
        previousWork.trend === Trend.UPWARD &&
        work.trend === Trend.DOWNWARD &&
        nextWork.trend === Trend.UPWARD
